...
const result = await client.query("UPDATE cms_users SET password_hash=$1, active=true WHERE username=$2", [hash, username]);
if (!result.rowCount) {
  await client.end();
  throw new Error(`CMS user '${username}' does not exist. Run npm run cms:setup first.`);
}
