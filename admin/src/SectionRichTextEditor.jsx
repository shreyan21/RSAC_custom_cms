...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, Heading2, Heading3, Heading4, Italic, Link2, List, ListOrdered, Minus, Quote, Redo2, RemoveFormatting, Sparkles, Strikethrough, Table2, Underline, Undo2, Unlink } from "lucide-react";
...
const RsacTextStyle = Mark.create({
  name: "rsacTextStyle",
  parseHTML: () => [{ tag: "span[data-rsac-font], span[data-rsac-size]" }],
...
const SectionRichTextEditor = forwardRef(function SectionRichTextEditor({ value, onChange, ariaLabel }, ref) {
  ...
  const updateTextStyle = (name, nextValue) => {
    ...
    if (!next.fontFamily && !next.fontSize) editor.chain().focus().unsetMark("rsacTextStyle").run();
  ...
  return (
    <div className="section-rich-editor">
      <div className="section-rich-editor__toolbar" role="toolbar" aria-label={`${ariaLabel} formatting`}>
        ...
        <div className="section-rich-editor__toolgroup"><ToolbarButton label="Bold" active={state.bold} onClick={() => editor.chain().focus().toggleBold().run()}><Bold /></ToolbarButton><ToolbarButton label="Italic" active={state.italic} onClick={() => editor.chain().focus().toggleItalic().run()}><Italic /></ToolbarButton><ToolbarButton label="Underline" active={state.underline} onClick={() => editor.chain().focus().toggleUnderline().run()}><Underline /></ToolbarButton><ToolbarButton label="Strikethrough" active={state.strike} onClick={() => editor.chain().focus().toggleStrike().run()}><Strikethrough /></ToolbarButton></div>
        <div className="section-rich-editor__selectgroup"><label><span>Font</span><select aria-label="Selected text font family" value={state.fontFamily} onChange={(event) => updateTextStyle("fontFamily", event.target.value)}>{fontOptions.map(([optionValue, label]) => <option value={optionValue} key={label}>{label}</option>)}</select></label><label><span>Size</span><select aria-label="Selected text size" value={state.fontSize} onChange={(event) => updateTextStyle("fontSize", event.target.value)}>{sizeOptions.map(([optionValue, label]) => <option value={optionValue} key={label}>{label}</option>)}</select></label></div>
        ...
        <div className="section-rich-editor__toolgroup"><ToolbarButton label="Horizontal divider" onClick={() => editor.chain().focus().setHorizontalRule().run()}><Minus /></ToolbarButton><ToolbarButton label="Clear formatting" onClick={() => editor.chain().focus().unsetAllMarks().clearNodes().unsetRsacTextAlign().run()}><RemoveFormatting /></ToolbarButton><ToolbarButton label="Format text" disabled={editor.isEmpty} onClick={formatText}><Sparkles /><span>Format text</span></ToolbarButton></div>
      </div>
      <EditorContent editor={editor} />
      <p className="section-rich-editor__help">Select text to change its font, size, emphasis, alignment, heading, list, link, or table. Changes stay inside this language and section.</p>
    </div>
  );
});
